...
- [memory-boundary] **Memory use boundaries:** Treat memories as historical leads; they must not override current explicit instructions or effective specifications. Verify relevant facts when relying on memories about current behavior, configuration, or versions, and do not directly apply conventions from another project. Do not infer authorization for the current task solely from historical memories or reinstate rules or preferences that have been explicitly revoked.

- [repo-rule-extraction] **Proactive rule extraction:** After resolving a problem in response to a user instruction, the AI may proactively derive a repository rule without separate approval when the problem reveals a reusable error or practice pattern likely to recur. Read and follow [`rules-and-skills-authoring.md`](rules-and-skills-authoring.md) when doing so. Keep the rule concise and independently complete, with clear applicability and necessary exceptions; do not turn one-time decisions or generic coding advice into rules. Historical memories may suggest candidates, but memory summaries alone do not justify creating rules: require verifiable user requirements or evidence from resolving the problem, and confirm that the rule remains applicable to the current repository. Check existing applicable rules to avoid conflicts; do not add a duplicate when an existing rule already covers the constraint. Add qualifying rules to `AI-coding-specification/repo-rules.md` under the affected repository root, creating the directory or file only when needed. The destination is the repository where the problem was resolved, not the installed specification directory. After creating a rule, the AI must tell the user what was added, which recurring pattern it addresses, and where it was saved. Extraction is optional, not a mandatory step after every task.
...
- [try-catch] Let exceptions propagate by default. Catch only when the current layer has a clear error-handling responsibility.
  - **Responsibility:** Catch only for defined recovery or continuation flows, centralized error-boundary handling, or business rollback or compensation. Call sites must not duplicate logging, formatting, or presentation already owned by a centralized boundary, or catch solely to rethrow the same error unchanged.
  - **Scope:** Enclose only the operations that need handling. Let errors the current layer cannot handle propagate, and keep subsequent unrelated operations outside the catch scope.
  - **Cleanup:** Use `finally` for state resets, resource release, and other work required on both success and failure. Do not add a `catch` solely for cleanup.
  - **Failure semantics:** Unless recovery or continuation is explicitly allowed, failure must still propagate after handling. Logging, showing a notification, or setting error state does not mean success; do not conceal failure with an empty catch, a default value, or a normal return.
  - **Equivalent forms:** Apply the same rules to `Promise.catch()`, rejection callbacks, and error-handling wrappers. Do not evade the rules by changing syntax or introduce an abstraction solely to eliminate a `catch`.
