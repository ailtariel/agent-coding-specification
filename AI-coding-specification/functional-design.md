...
- Implementation tasks remain governed by [`coding-specification.md`](coding-specification.md). Design-only requests do not authorize implementation. Existing end-to-end implementation authorization remains valid through design refinement.
...
- [design-conflict] Resolve conflicts using explicit instruction priority first. When the current user clearly changes a prior decision, proceed with that direction and update relevant documents. Request a decision only for remaining ambiguity affecting the design; continue independent work.
...
- [user-decision-gate] For the following material decisions not already settled or authorized by the user, provide analysis and a preferred option and request a decision:
