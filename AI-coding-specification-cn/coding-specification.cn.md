...
- [memory-boundary] **记忆使用边界：**记忆仅作为历史线索，不得覆盖当前明确指令和有效规范。依赖记忆判断当前行为、配置或版本时，应核对相关事实，不将其他项目的约定直接套用。不得仅凭历史记忆推定当前任务已获授权，或恢复已被明确撤销的规则与偏好。

- [repo-rule-extraction] **主动提炼仓库规则：**基于用户指令解决问题后，如果该问题体现了未来很可能再次出现、可复用的错误或实践范式，AI 可以主动提炼仓库规则，无需另行请求批准。提炼时读取并遵循 [`rules-and-skills-authoring.cn.md`](rules-and-skills-authoring.cn.md)。规则应简练、独立完整，写清适用范围和必要例外，不得将一次性决策或通用编码知识写成规则。历史记忆可以提供候选线索，但不得仅凭记忆摘要创建规则；应有可核实的用户要求或问题处理证据，并确认其仍适用于当前仓库。先检查已有适用规则，避免冲突；已有规则覆盖时，不重复新增。将符合条件的规则加入受影响仓库根目录下的 `AI-coding-specification/repo-rules.md`，仅在需要时创建目录或文件。目标是实际解决该问题的仓库，不是规范的安装目录。创建后必须告知用户新增了什么规则、针对什么可重复出现的范式，以及保存位置。提炼是可选行为，不是每次任务后的必做步骤。
...
- [try-catch] 默认让异常向上传播，仅在当前层承担明确的错误处理职责时捕获。
  - **捕获职责：**仅用于已定义的恢复或继续流程、统一错误边界处理、业务回滚或补偿。已有统一边界负责的记录、格式化和展示，调用处不得重复处理，也不得仅捕获后原样重抛。
  - **捕获范围：**只包围需要处理的操作；当前层无法处理的错误继续传播，不纳入后续无关操作。
  - **清理方式：**无论成功失败都要执行的状态复位、资源释放等操作使用 `finally`，不为清理增加 `catch`。
  - **失败语义：**除非明确允许恢复或继续执行，否则处理后仍须传播失败；记录日志、展示通知或设置错误状态不代表成功，不得用空捕获、默认值或正常返回掩盖失败。
  - **适用范围：**同样约束 `Promise.catch()`、rejection 回调和错误处理包装函数；不得通过换写法规避，也不得仅为消除 `catch` 新增抽象层。
