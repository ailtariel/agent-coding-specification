...
## Rules
- Check availability with the current shell (PowerShell: `Get-Command antd`; POSIX: `command -v antd`). Use an existing installation. Install only when requested or already authorized.
- An update notice does not require a global upgrade. Use version-matched official documentation when the installed tool cannot answer.
- Always use `--format json`.
- Match the project version with `--version <x.y.z>` when needed.
- Query uncertain or version-sensitive APIs; reuse verified answers.
- Use `antd lint` on affected paths when it adds relevant verification; follow project verification requirements.
...
### antd component bugs
Only for a requested bug-reporting task: prepare a preview, then submit if authorized.
...
### CLI bugs
A CLI failure does not automatically trigger reporting. Continue the main task with authoritative alternative sources. Prepare a report when requested; sanitize project details and submit only when authorized.
